...
impl MeilisearchService {
    ...
    pub fn new(host: &str, api_key: &str) -> Self {
        let client = Client::new(host, Some(api_key)).expect("Failed to create Meilisearch client");
        Self { client }
    }
        ...
        let version =
            self.client
                ...
                .map_err(|e: meilisearch_sdk::errors::Error| {
                    ...
                    {
                        "API Key 无效或权限不足，请检查配置".to_string()
                    } else {
    ...
    pub async fn get_task_status(&self, task_uid: i64) -> Result<String, String> {
        ...
        let task = tasks.results.into_iter().find(|t| match t {
